...
export function HomePage() {
  ...
  const healthScore = relevantDevices.length > 0 
    ? Math.round(((relevantDevices.filter(d => d.status === 'online').length) / relevantDevices.length) * 100) 
    : 0;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12 space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          {lastUpdated && (
            <p className="text-[10px] font-mono text-slate-600 uppercase">Last Sync: {new Date(lastUpdated).toLocaleTimeString()} • Real-time Monitoring Active</p>
          )}
        </div>
        <div className="flex gap-3">
          ...
          >
            ...
            {isExporting ? 'Generating Archive...' : 'Export Compliance Archive'}
          </Button>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          ...
          { label: 'Fleet Health Score', value: `${healthScore}%`, color: 'text-emerald-400', ping: healthScore > 95 },
          { label: 'Online Frequency', value: onlineCount, color: 'text-blue-400' },
          ...
          <Card key={i} className="bg-slate-900 border-white/5 shadow-xl hover:bg-slate-900/80 transition-colors group">
            <CardHeader className="pb-2">
              <CardTitle className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex justify-between items-center">
                {stat.label}
                {stat.ping && <span className="h-1.5 w-1.5 bg-emerald-500 rounded-full animate-ping" />}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className={cn("text-3xl font-bold font-mono tracking-tighter group-hover:scale-105 transition-transform origin-left", stat.color)}>
                {stat.value}
              </div>
            </CardContent>
          </Card>
      ...
      <div className="grid gap-8 lg:grid-cols-4">
        <div className="lg:col-span-3 space-y-4">
          <div className="flex items-center gap-4 bg-slate-950 p-4 rounded-xl border border-white/5 shadow-inner">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Query fleet by name, system ID, or node IP address..."
                className="pl-9 bg-slate-900 border-white/10 text-xs h-9 focus:ring-1 focus:ring-blue-500/50 transition-colors placeholder:text-slate-600"
                value={search}
              ...
              <TableBody>
                {filteredDevices.map(device => (
                  <TableRow key={device.id} className="border-white/5 hover:bg-blue-500/[0.03] transition-colors group">
                    ...
                    <TableCell>
                       <span className="text-[10px] font-mono text-slate-500">{new Date(device.lastSeen).toLocaleTimeString([], {hour12: false})}</span>
                    </TableCell>
                    <TableCell>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              ...
              {fleetActivity.length === 0 ? (
                ...
                <>
                  ...
                  <div className="py-6 text-center">
                    <span className="text-[9px] font-mono text-slate-800 uppercase tracking-widest">Buffer Horizon Reached</span>
                  </div>
                </>
              )}
            </AnimatePresence>

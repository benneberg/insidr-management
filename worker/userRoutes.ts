...
import type { ApiResponse, Device, SystemAlert, CDPLiteV2Payload } from '@shared/types';
...
(function(){const n="node-"+Math.random().toString(36).slice(2,7);const c=localStorage.getItem('insidr-consent')==='true';if(c)fetch("/api/devices/"+n+"/ingest",{method:"POST",headers:{"X-Transport":"JSON"},body:JSON.stringify({params:{deviceId:n,logs:[{level:"info",message:"Agent check-in"}]},sequence:1})})})();`;
